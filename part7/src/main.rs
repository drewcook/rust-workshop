...
// Using lifetime annotations so we can return slices from borrowed values
// Using just <'a> is saying, all things will have the same lifetime here, but it is more constrictive.
// Using <'a, 'b> actually allows more flexibility and can be less error prone.
